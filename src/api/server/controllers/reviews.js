...
class ReviewsController {
  ...
  static create(req, res, next) {
    ...
    req.checkBody("product").notEmpty().withMessage(" Product should not be empty");
    req.checkBody("review").notEmpty().withMessage(" Comment should not be empty");
    ...
    body.customer = req.user.customers;
    body.created_at = now;
    ...
    ReviewDal.create(body, (err, doc) => {
      ...
      }, options, function updatecommissionpayment(err, idoc) {
        if (err) {
          return next(err);
        }
      });
      InternalDal.get({
        brands: doc.product.brands
      }, function showSalers(err, internalDoc) {
        if (err) {
          return next(err);
        }
        ...
        const itemQury = {
          ...
          message: `${req.user.email} has made a review on your product ${doc.product.name}. Please go to your product section and view what was commented on. <BR> <img src="'${doc.img}'" style="width: 55px; margin-right: 12px;"/> ${doc.review}`,
          salers: internalDoc._id,
          created_at: now,
          created_by: req.user.customers,
          is_read: false,
  ...
  static delete(req, res, next) {
    ReviewDal.delete({
      ...
      CustomerDal.update({
        _id: req.user.customers
      }, options, function updatecommissionpayment(err, doc) {
        if (err) {
          return next(err);
        }
      });
