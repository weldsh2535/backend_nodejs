...
class BrandController {
  ...
  static sales(req, res, next) {
    const query = {
      _id: req.doc.products.brands._id
    };
    const options = {};
  ...
  static fetchOne(req, res, next) {
    ...
    InternalDal.get({
      ...
      ShowCaseDal.getCollection({
        brand: req.doc._id
      }, options, function getAll(err, iShows) {
        if (err) {
          return next(err);
        }
    ...
    BrandsDal.get({
      ...
      switch (cats.followers.length) {
        case 0:
          BrandsDal.update({
            _id: req.doc._id
          }, follow, function UpdateToFollow(err, doc) {
            if (err) {
              return next(err);
            }
        ...
        default:
          ...
          if (checkUser >= 0) {
            BrandsDal.update({
              _id: req.doc._id
            }, unfollow, function UpdateToUnfollow(err, doc) {
              if (err) {
                return next(err);
              }

              res.status(200).json({
                msg: "Unfollowed Brand",
                status: 200
              });
            });
          } else {
            BrandsDal.update({
              _id: req.doc._id
            }, follow, function UpdateToUnfollow(err, doc) {
              if (err) {
                return next(err);
              }
    ...
    BrandsDal.get({
      ...
      switch (cats.likes.length) {
        case 0:
          BrandsDal.update({
            _id: req.doc._id
          }, ilike, function UpdatetoLike(err, doc) {
            if (err) {
              return next(err);
            }
        ...
        default:
          if (checkUser >= 0) {
            BrandsDal.update({
              _id: req.doc._id
            }, unlike, function UpdateToUnLike(err, doc) {
              if (err) {
                return next(err);
              }
            ...
            BrandsDal.update({
              _id: req.doc._id
            }, ilike, function UpdateToUnLike(err, doc) {
              if (err) {
                return next(err);
              }
  ...
  static create(req, res, next) {
    ...
    req.checkBody("title").notEmpty().withMessage(" title name should not be empty");
