...
function removeFromCart(req, res, next) {
  ...
  ProductDal.getCollection({}, options, function getAll(err, cats) {
    ...
    async.eachSeries(cats, (data, callback) => {
      const deleteID = data._id;
      SalesDal.getCollection({
        products: data._id
      }, options, function getAll(err, iSales) {
        if (err) {
          return next(err);
        }
        ...
        if (differences <= 0) {
          CartsDal.getCollection({
            product: data._id
          }, options, function getAll(err, iCarts) {
            if (err) {
              return next(err);
            }
          ...
          InternalDal.get({
            _id: data.created_by.internal
          }, function getAll(err, iUsers) {
            if (err) {
              return next(err);
            }
            ...
            NotifiDal.getCollection(notquery, options, function getAll(err, checkNotification) {
              if (err) {
                return next(err);
              }
...
class CheckoutController {
  ...
  static showlists(req, res, next) {
    ...
    Checkout.getCollection({
      customer: req.user.customers
    }, options, function getAll(err, cats) {
      if (err) {
        return next(err);
      }
  ...
  static update(req, res, next) {
    ...
    Checkout.update({
      _id: req.doc._id
    }, body, function updatecommissionpayment(err, doc) {
      if (err) {
        return next(err);
      }
  ...
  static create(req, res, next) {
    ...
    req.checkBody("billing").notEmpty().withMessage(" Billing Address should not be empty");
    req.checkBody("mobile").notEmpty().withMessage(" Mobile should not be empty");
    req.checkBody("country").notEmpty().withMessage(" country should not be empty");
    req.checkBody("state").notEmpty().withMessage(" state should not be empty");
    req.checkBody("zipcode").notEmpty().withMessage(" zipcode should not be empty").len(4).withMessage("4 digit code");
    req.checkBody("cardname").notEmpty().withMessage(" Card Name should not be empty");
    req.checkBody("cardnumber").notEmpty().withMessage(" Card Number should not be empty");
    req.checkBody("cardExpirmont").notEmpty().withMessage(" Card Exiration Month should not be empty").len(1, 2).withMessage("Enter the month number maximum two digit number");
    req.checkBody("cardExpirYear").notEmpty().withMessage(" Card Exiration Year should not be empty").len(4).withMessage("Enter the Year must be Four digits");
    ...
    const options = {};
    CustomerDal.get({
      _id: req.user.customers
    }, function getAll(err, iUser) {
      if (err) {
        return next(err);
      } // this is for stripe
      ...
      Stripe.tokens.create({
        ...
        email: req.user.customers.email,
        source: token.id
        ...
        CartsDal.getCollection({
          ...
          for (i = 0; i < iCart.length; i++) {
            ...
            body.checkouts = saved._id, body.brands = iCart[i].product.brands._id;
      ...
      })).then(charge => stripeResponse = charge)).catch(err => {
