...
class CustomersController {
  ...
  static updateCustomer(req, res, next) {
    ...
    CustomerDal.update({
      _id: req.doc._id
    }, body, function updateCl(err, doc) {
      if (err) {
        return next(err);
      }
  ...
  static changeLanguage(req, res, next) {
    const body = req.body;
    req.checkBody("language", "Language  should not be empty!").withMessage("Should not be empty").notEmpty();
    const validationErrors = req.validationErrors();
  ...
  static uploadProfile(req, res, next) {
    const dest = `uploads/${req.files[0].filename}`; // var dest = 'uploads/' + (Math.random().toString(36)+'00000000000000000').slice(2, 10) + Date.now() + req.files[0].filename;
  ...
  static getPaginated(req, res, next) {
    ...
    CustomerDal.getCollectionBy(query, queryOpts, function getByPaginationCb(err, doc) {
      if (err) {
        return next(err);
      }
