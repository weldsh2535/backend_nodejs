...
class NotificationController {
  ...
  static create(req, res, next) {
    ...
    req.checkBody("message").notEmpty().withMessage("Please enter the message"); // training_date
    ...
    body.created_at = now;
    body.created_by = req._user._id; // body.helper = req._user.helper;
  ...
  static broadCastedMessages(req, res, next) {
    const options = {};
    const noCustomerHelper = {
      customers: null,
      helper: null,
      broadcast: true
    };
    NotifiDal.getCollection(noCustomerHelper, options, function getAll(err, cats) {
      if (err) {
        return next(err);
      }
  ...
  static update(req, res, next) {
    ...
    NotifiDal.update({
      _id: req.doc._id
    }, body, function updateNotifi(err, doc) {
      if (err) {
        return next(err);
      }
