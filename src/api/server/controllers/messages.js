...
class MessageController {
  ...
  static fetchOne(req, res, next) {
    const updateQuery = {
      is_readCustomer: true
    };
    MessagesDal.update({
      _id: req.doc._id
    }, updateQuery, function updatecommissionpayment(err, doc) {
      if (err) {
        return next(err);
      }
    });
  ...
  static create(req, res, next) {
    ...
    req.checkBody("message").notEmpty().withMessage("message should not be empty");
    const validationErrors = req.validationErrors();
    ...
    SalersDal.get({
      ...
      MessagesDal.getCollection(query, options, function getAll(err, cats) {
        ...
        switch (Object.keys(cats).length) {
          case 0:
            req.checkBody("salers").notEmpty().withMessage("Salers id must be requried");
            req.checkBody("brands").notEmpty().withMessage("brands must be requried");
          ...
          default:
            MessagesDal.getCollection(query, options, function getAll(err, iLastRecord) {
              if (err) {
                return next(err);
              }
