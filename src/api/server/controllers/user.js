...
class UserController {
  ...
  static createUser(req, res, next) {
    ...
    workflow.on("validateUser", function validateUser() {
      // debug('validate user');
      // Validate User Data
      req.checkBody("username", "Username  should not be empty!").isEmail().withMessage("Username should be email").notEmpty();
      ...
      req.checkBody("user_type", "User Type is Invalid!").notEmpty().withMessage("User Type should not be Empty").isIn(["super_admin", "admin", "data_encoder", "customers", "salers"]).withMessage("Invalid User type"); // if org--> Sector
      ...
      */
      //console.log("DATA BODY::"+body.username);
      const validationErrors = req.validationErrors();
    ...
    workflow.on("checkUserExist", function checkUserExist() {
      ...
      const username = body.username; // Query DB for a user with the given ID
    ...
    workflow.on("createUserType", function respond(user) {
      if (body.user_type === "admin") {
        req.checkBody("myRole", "Role is invalid!").notEmpty().withMessage("Your Role should not be Empty");
        req.checkBody("my_id", "User ID is Invalid!").notEmpty().withMessage("Your User ID should not be Empty");
        ...
        body.created_at = now;
        body.role = body.role;
        body.email = body.username; // uniqune ID generation is over
        ...
        body.user = user._id;
        body.created_at = now;
        body.role = body.role; // uniqune ID generation is over
  ...
  static createAdmin(req, res, next) {
    ...
    req.checkBody("username", "Username  should not be empty!").isEmail().withMessage("Username should be email").notEmpty();
    req.checkBody("password").notEmpty().withMessage("password should not be empty").len(6, 20).withMessage("6 to 20 characters required");
    req.checkBody("user_type", "User Type is Invalid!").notEmpty().withMessage("User Type should not be Empty").isIn(["super_admin", "corporate", "helper", "admin", "customers"]).withMessage("User Type should either be client or organization");
    const username = body.username;
    body.created_at = now;
    body.role = body.role; // Query DB for a user with the given ID
        ...
        res.json({
          ...
          UserDal.create({
            ...
            if (err) {
              ...
              body.user = users._id;
              body.created_by = req._user._id; // body.created_by = req._user.id;
  ...
  static passwordChange(req, res, next) {
    ...
    workflow.on("checkPassword", function checkPassword(user) {
      user.checkPassword(body.old_password, function check(err, isOk) {
        ...
        if (!isOk) {
          res.status(403); //    console.log(body.old_password);
  ...
  static getByPagination(req, res, next) {
    const query = {}; // retrieve pagination query params
  ...
  static resetPhonePass(req, res, next) {
    ...
    req.checkBody("new_password").notEmpty().withMessage("new password should not be empty").len(6, 20).withMessage("6 to 20 characters required");
    req.checkBody("confirmed_password").notEmpty().withMessage("confirm password should not be empty").len(6, 20).withMessage("6 to 20 characters required");
  ...
  static forgotPassword(req, res, next) {
    ...
    const resetLinkAddress = settings.PASSWORD_RESET.RESET_LINK_ADDRESS; // const now = new Date()
