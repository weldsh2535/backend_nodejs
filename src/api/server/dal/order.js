...
class OrderDal {
  ...
  static get(query, cb) {
    Order.findOne(query).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
  ...
  static delete(query, cb) {
    Order.findOne(query).populate(population).exec(function deleteOrder(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getCollection(query, qs, cb) {
    Order.find(query, {}, qs).sort({
      _id: -1
    }).populate(population).exec(function getOrderCollection(err, doc) {
      if (err) {
        return cb(err);
      }
