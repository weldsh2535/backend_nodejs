...
class MessagesDal {
  ...
  static get(query, cb) {
    Message.findOne(query).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
  ...
  static delete(query, cb) {
    Message.findOne(query).populate(population).exec(function deleteMessage(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static update(query, updates, cb) {
    ...
    Message.findOneAndUpdate(query, updates, opts).populate(population).exec(function updateMessage(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static updateBulk(query, updates, cb) {
    Message.updateMany(query, updates).exec(function updateMessage(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getCollection(query, opt, cb) {
    Message.find(query, {}, opt).populate(population).exec(function getMessagesCollection(err, doc) {
      if (err) {
        return cb(err);
      }
