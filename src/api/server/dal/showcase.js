...
class ShowCaseDal {
  ...
  static get(query, cb) {
    ShowCase.findOne(query).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
  ...
  static delete(query, cb) {
    ShowCase.findOne(query).populate(population).exec(function deleteShowCase(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static update(query, updates, cb) {
    ...
    ShowCase.findOneAndUpdate(query, updates, opts).populate(population).exec(function updateShowCase(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getCollection(query, opt, cb) {
    ShowCase.find(query, {}, opt).populate(population).exec(function getShowCasesCollection(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getPaginatedResults(query, page, limit, cb) {
    ...
    ShowCase.paginate(query, options, function getShowCasesCollection(err, docs) {
      if (err) {
        return cb(err);
      }
