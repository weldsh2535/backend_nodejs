...
class UserDal {
  ...
  static delete(query, cb) {
    User.findOne(query, returnFields).populate(population).exec(function deleteUser(err, user) {
      if (err) {
        return cb(err);
      }
  ...
  static update(query, updates, cb) {
    ...
    User.findOneAndUpdate(query, updates, opts).populate(population).exec(function updateUser(err, user) {
      if (err) {
        return cb(err);
      }
  ...
  static get(query, cb) {
    User.findOne(query).populate(population).exec((err, user) => {
      if (err) {
        console.log(err);
        return cb(err);
      }
  ...
  static getCollection(query, qs, cb) {
    User.find(query, {}, qs).populate(population).exec((err, user) => {
      if (err) {
        return cb(err);
      }
