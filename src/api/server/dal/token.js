...
class TokenDal {
  ...
  static get(query, cb) {
    Token.findOne(query, returnFields).populate(population).exec(function getToken(err, token) {
      if (err) {
        return cb(err);
      }
  ...
  static deleteItem(query, cb) {
    const opts = {};
    Token.findOne(query, opts).populate(population).exec(function deleteToken(err, token) {
      if (err) {
        return cb(err);
      }
  ...
  static update(query, updates, cb) {
    ...
    Token.findOneAndUpdate(query, updates, opts).populate(population).exec(function updateToken(err, token) {
      if (err) {
        return cb(err);
      }
  ...
  static getCollection(query, qs, cb) {
    cb(null, Token.find(query, returnFields).populate(population).stream({
