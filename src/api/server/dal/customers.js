...
class CustomersDal {
  ...
  static get(query, cb) {
    Customers.findOne(query).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
  ...
  static delete(query, cb) {
    Customers.findOne(query, returnFields).populate(population).exec(function deleteUser(err, Customers) {
      if (err) {
        return cb(err);
      }
  ...
  static update(query, updates, cb) {
    ...
    Customers.findOneAndUpdate(query, updates, opts).populate(population).exec((err, cust) => {
      if (err) {
        return cb(err);
      }
  ...
  static getCollection(query, qs, cb) {
    Customers.find(query, {}, qs).sort({
      _id: -1
    }).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
