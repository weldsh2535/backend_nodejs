...
class TimeLineDal {
  ...
  static get(query, cb) {
    TimeLine.findOne(query).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
  ...
  static delete(query, cb) {
    TimeLine.findOne(query).populate(population).exec(function deleteOrder(err, doc) {
      if (err) {
        return cb(err);
      }
