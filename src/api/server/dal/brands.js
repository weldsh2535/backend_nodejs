...
class BrandDal {
  ...
  static get(query, cb) {
    Brands.findOne(query).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
  ...
  static delete(query, cb) {
    Brands.findOne(query).populate(population).exec(function deleteBrands(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static update(query, updates, cb) {
    ...
    Brands.findOneAndUpdate(query, updates, opts).populate(population).exec(function updateBrands(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getCollection(query, opt, cb) {
    Brands.find(query, {}, opt).sort({
      title: 1
    }).populate(population).exec(function getBrandssCollection(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getPaginatedResults(query, page, limit, cb) {
    ...
    Brands.paginate(query, options, function getFollowersCollection(err, docs) {
      if (err) {
        return cb(err);
      }
