...
class InternalDal {
  ...
  static get(query, cb) {
    Internal.findOne(query).populate(population).exec((err, doc) => {
      if (err) {
        return cb(err);
      }
  ...
  static delete(query, cb) {
    Internal.findOne(query).populate(population).exec(function deleteinternal(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static update(query, updates, cb) {
    ...
    Internal.findOneAndUpdate(query, updates, opts).populate(population).exec(function updateinternal(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getCollection(query, opt, cb) {
    Internal.find(query, opt).populate(population).exec(function getinternalsCollection(err, doc) {
      if (err) {
        return cb(err);
      }
  ...
  static getPaginatedResults(query, page, limit, cb) {
    ...
    Internal.paginate(query, options, function getFollowersCollection(err, docs) {
      if (err) {
        return cb(err);
      }
