...
(async () => {
  try {
    ...
    winston.error(`MongoDB - Store - Sartor connection was failed. ${error.message}`);
    return;
  }
})(); // MongoDB error handler.


mongoose.connection.on("error", Utils.mongoErrorHandler); // MongoDB Disconnection handler.

mongoose.connection.on("disconnected", async function handleMongodbDisconnection() {
  await mongoose.connect(settings.MONGODB.URL, settings.MONGODB.OPTS);
}); // Service Settings

app.disable("x-powered-by"); // PRODUCTION Environment settings
...
app.use("/fullcalendar", express.static(`${process.cwd()}/public/sartor/fullcalendar`));
...
app.use(session({
  secret: settings.cookieSecretKey,
  resave: false,
...
initRoutes(app); // catch 404 and forward to error handler
